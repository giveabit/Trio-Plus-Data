...
def getAudioBlocks(zeroBlocks, data, fileSize, debugFile=''):
    ...
    if fileSize-zeroBlocks[-1]>chunkSize:
    ...
    audioBlocksTemp = detectNewSongParts(audioBlocksTemp)
...
def detectNewSongParts(audioBlocksTemp):
...
for fileName in fileList:
    ...
    if debug:
        fileSize, zeroBlocks = getZeroBlocks(data, debugDir+'/'+fileName+'.DBG.txt')
        audioBlocks = getAudioBlocks(zeroBlocks, data, fileSize, debugDir+'/'+fileName+'.DBG.txt')
    else:
        fileSize, zeroBlocks = getZeroBlocks(data)
        audioBlocks = getAudioBlocks(zeroBlocks, data, fileSize)
