...
def findAudioStart(data, debugFile):
    ...
    if start:
        ...
        if start > offsetAudioDefault:
            return offsetAudioDefault #audio really seems to start at 170800 bytes
        else:
            return start #in case overdub recorded in first part might start earlier
    else:
...
def getZeroBlocks(data, start, debugFile):
    fileSize = len(data)
    zeroBlocks = []
    position = start
...
def getAudioBlocks(zeroBlocks, data, start, fileSize, debugFile):

    # push in the start element and the end element so that
    ...
    for index in range(0,len(audioBlocksTemp),2):
        ...
        if lenght > chunkSize+margin:
            if lenght > 2 * chunkSize:  # ...a HUGE block is likely to be separate part on it's own
                if index == 0: # huge at the very beginning
                    marker.append(index + 2)
                elif index+2 == len(audioBlocksTemp): # huge at the very end
                    marker.append(index)
                else: # huge in between
                    marker.append(index)
                    marker.append(index + 2)
...
for fileName in fileList:
    ...
    start = findAudioStart(data, debugFile)
    fileSize, zeroBlocks = getZeroBlocks(data, start, debugFile)
